...
public record MeResponse(UserSummary user, TenantSummary tenant) {
    ...
    public record UserSummary(UUID id, String fullName, String email, boolean emailVerified,
                              String role, String staffRole, String initials) {
    }
    ...
    public static MeResponse from(User user, Tenant tenant) {
        UserSummary userSummary = new UserSummary(
                user.getId(), user.getFullName(), user.getEmail(), user.isEmailVerified(),
                user.getRole(), user.getStaffRole(), initials(user.getFullName()));
